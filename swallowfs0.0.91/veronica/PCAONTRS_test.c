...
int PCAONTRS_test_set_address(SHAREADDRESS *addptr,int n)
{
...
int PCAONTRS_test()
{
	...
	if(content==NULL)
	{
		show_error("PCAONTRS","PCAONTRS_test","No memory");
		return FALSE;
	}
	...
	k=4;
	fout=fopen(".\\PCAONT-RS.txt","w");
	if(fout==NULL)
	{
		show_error("PCAONTRS","PCAONTRS_test","output file cannot open");
		return FALSE;
	}
	for(r=0;r<100;r++)
	{
		for(i=0;i<7;i++)
		{
			...
			for(k=4;k<=7;k++)
			{
				...
				for(itemindex=0;itemindex<randnum;itemindex++)
				{
					...
					if(resTF==FALSE)
					{
						show_error("PCAONTRS","PCAONTRS_test","the traceitem cannot be read");
						return FALSE;
					}
				}
				//while(!feof(fin))
				for(itemindex=0;itemindex<100;itemindex++)
				{
					...
					strlen=itemptr->length;
					PCAONTRS_API(content,strlen,addlistptr);
					endtick=clock();
					sectime=profile_calculate_time_diff(starttick,endtick);
					fprintf(fout,"%f\n",sectime);
				}
