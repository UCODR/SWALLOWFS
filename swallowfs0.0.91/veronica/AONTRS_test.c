...
int AONTRS_test_set_address(SHAREADDRESS *addptr,int n)
{
...
int AONTRS_test()
{
	...
	if(content==NULL)
	{
		show_error("AONTRS","AONTRS_test","No memory");
		return FALSE;
	}
	...
	for(r=0;r<100;r++)
	{
		for(i=0;i<7;i++)
		{
			...
			if(fin==NULL)
			{
				show_error("AONTRS","AONTRS_test","trace cannot open");
				return FALSE;
			}
