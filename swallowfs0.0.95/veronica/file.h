...
//原始数据读写接口
extern int file_FILELAYER_BYTE_READER_ORG(char *filepath,BYTE *content,int sizenum);
extern int file_FILELAYER_BYTE_WRITER_ORG(char *filepath,BYTE *content,int sizenum);
extern int file_FILELAYER_BYTE_DELETE_ORG(char *filepath);
//文件必须存在
...
extern int file_FILELAYER_CHUNKGROUP_READER_DATA(CHUNKGROUP *chkgroup);//读取数据块

extern int file_FILELAYER_CHUNKGROUP_READER_DATA_SEQ(CHUNKGROUP *chkgroup);//串行将CHUNKGROUP根据其中包含的路径读取到相应的区域,数据部分
extern int file_FILELAYER_CHUNKGROUP_READER_DATA_PARAL(CHUNKGROUP *chkgroup);//并行将CHUNKGROUP根据其中包含的路径读取到相应的区域,数据部分

extern int file_FILELAYER_CHUNKBLOCKGROUP_READER_DATA(CHUNKBLOCKGROUP *chkblkgroup);//总接口

extern int file_FILELAYER_CHUNKBLOCKGROUP_READER_DATA_SEQ(CHUNKBLOCKGROUP *chkblkgroup);//根据不同格式block进行串行
extern int file_FILELAYER_CHUNKBLOCKGROUP_READER_DATA_PARAL(CHUNKBLOCKGROUP *chkblkgroup);//根据不同格式block进行并行

extern int file_FILELAYER_CHUNKPOSITIONBLOCKGROUP_DATA_READER(CHUNKPOSITIONBLOCKGROUP *chkposblkgroup);//总接口

extern int file_FILELAYER_CHUNKPOSITIONBLOCKGROUP_READER_DATA_SEQ(CHUNKPOSITIONBLOCKGROUP *chkposblkgroup);//需要根据数据进行串行化读取
extern int file_FILELAYER_CHUNKPOSITIONBLOCKGROUP_READER_DATA_PARAL(CHUNKPOSITIONBLOCKGROUP *chkposblkgroup);//需要根据数据进行并行化读取

extern unsigned int __stdcall readthread(void *chkptr);//回调函数
...
extern int file_FILELAYER_CHUNKGROUP_WRITER_DATA(CHUNKGROUP *chkgroup);//总接口

extern int file_FILELAYER_CHUNKGROUP_WRITER_DATA_SEQ(CHUNKGROUP *chkgroup);//串行将CHUNKGROUP根据其中包含的路径写入到相应的区域,数据部分
extern int file_FILELAYER_CHUNKGROUP_WRITER_DATA_PARAL(CHUNKGROUP *chkgroup);//并行将CHUNKGROUP根据其中包含的路径写入到相应的区域,数据部分

extern int file_FILELAYER_CHUNKBLOCKGROUP_WRITER_DATA(CHUNKBLOCKGROUP *chkblkgroup); //总接口

extern int file_FILELAYER_CHUNKBLOCKGROUP_WRITER_DATA_SEQ(CHUNKBLOCKGROUP *chkblkgroup);//串行将CHUNKBLOCKGROUP写入磁盘
extern int file_FILELAYER_CHUNKBLOCKGROUP_WRITER_DATA_PARAL(CHUNKBLOCKGROUP *chkblkgroup);//并行将CHUNKBLOCKGROUP写入磁盘

extern int file_FILELAYER_CHUNKPOSITIONBLOCKGROUP_WRITER_DATA(CHUNKPOSITIONBLOCKGROUP *chkposblkgroup);//总接口

extern int file_FILELAYER_CHUNKPOSITIONBLOCKGROUP_WRITER_DATA_SEQ(CHUNKPOSITIONBLOCKGROUP *chkposblkgroup);//串行写入CHUNKPOSITIONBLOCKGROUP
extern int file_FILELAYER_CHUNKPOSITIONBLOCKGROUP_WRITER_DATA_PARAL(CHUNKPOSITIONBLOCKGROUP *chkposblkgroup);//并行化写入CHUNKPOSITIONBLOCKGROUP

extern unsigned int __stdcall writethread(void *chkptr);//回调函数
