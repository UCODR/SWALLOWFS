...
int CAONTRS_test_set_address(SHAREADDRESS *addptr,int n)
{
	...
	for(r=0;r<100;r++)
	{
		for(i=0;i<7;i++)
		{
			...
			for(n=4;n<=7;n++)
			{
				...
				codeptr->codenum=n-k;
				codeptr->w=8;
				CAONTRS_test_set_address(addptr,n);
				//while(!feof(fin))太多了
				randnum=rand()%1000;
				for(itemindex=0;itemindex<randnum;itemindex++)
				{
					...
					if(resTF==FALSE)
					{
						show_error("CAONTRS","CAONTRS_test","the traceitem cannot be read");
						return FALSE;
					}
				}
				for(itemindex=0;itemindex<100;itemindex++)
				{
					...
					AONTRS_API(content,strlen,addlistptr);
					endtick=clock();
			...
			for(k=4;k<=7;k++)
			{
				...
				for(itemindex=0;itemindex<100;itemindex++)
				{
					...
					strlen=itemptr->length;
					CAONTRS_API(content,strlen,addlistptr);
					endtick=clock();
					sectime=profile_calculate_time_diff(starttick,endtick);
					fprintf(fout,"%f\n",sectime);
				}
