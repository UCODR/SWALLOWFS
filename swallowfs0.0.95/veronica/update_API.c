											...
											CHUNKBLOCKGROUP *parityblockgroupptr,BITMATRIX *matrix)
{
	...
	case UPDATEOPTINTERFACE:
		...
		break;
	case  UPDATE1INTERFACE:
		resTF=matrix_bitmatrix_updateI_stripsect_RAID6(chunkpositionblockgroupptr,strsetblockgroupptr,pparity,qparity,
												parityblockgroupptr,matrix);
		if(resTF==FALSE)
		{
			show_error("UPDATE_API","bitmatrix_update_RAID6","");
		...
		break;
	case UPDATE2INTERFACE:
		resTF=matrix_bitmatrix_updateII_stripsect_RAID6(chunkpositionblockgroupptr,strsetblockgroupptr,pparity,qparity,
												parityblockgroupptr,matrix);
		if(resTF==FALSE)
		{
			show_error("UPDATE_API","bitmatrix_update_RAID6","");
