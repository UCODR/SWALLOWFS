...
int profile_XOR_CHUNKBLOCK_CHUNK_DATA(CHUNK *tarchunkptr,CHUNKBLOCKGROUP *chunkblkgroup)
{
	CHUNKBLOCK *chunkblkptr,*curchunkblkptr;
	CHUNK *chunkptr;
	int i,resTF,groupsize;

	groupsize=chunkblkgroup->groupsize;
	if(groupsize==0)
	{
		return TRUE;
	}
	chunkblkptr=chunkblkgroup->chunkblockptr;

	chunkptr=chunkblkptr->chunkptr;//第一个块先copy一下
	resTF=matrix_CHUNK_copy_data(tarchunkptr,chunkptr);
	if(resTF==FALSE)
	{
		show_error("profile","XOR_CHUNKBLOCK_CHUNK_DATA","chunk data cannot copy");
		return FALSE;
	}

	
	curchunkblkptr=chunkblkptr+1;
...
int profile_XOR_CHUNKBLOCK_CHUNK_DATA_XOR(CHUNK *tarchunkptr,CHUNKBLOCKGROUP *chunkblkgroup)
{
	CHUNKBLOCK *chunkblkptr,*curchunkblkptr;
	CHUNK *chunkptr;
	int i,resTF,groupsize;
