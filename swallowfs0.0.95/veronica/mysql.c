...
int mysql_FILELAYER_checkpoint_select(CHECKPOINT *checkpointptr)
{
	UINT64 RecordID,requestID,
		regionID,stripID,strsetID,chunkID;
	int regionoffset,stripoffset,strsetoffset,chunkoffset;
	char *orgcontent,*upcontent;
	...
	RecordID=checkpointptr->RecordID;
	sprintf(querystr,
		"select RecordID,requestID,regionID,regionoffset,stripID,stripoffset,strsetID,strsetoffset,chunkID,chunkoffset,chunksize,orgcontent,upcontent from checkpoint where RecordID=%I64d",
		RecordID);
	mysqlresptr=mysql_FILELAYER_select_query(querystr);
	if(mysqlresptr==NULL)
	{
	...
	STRTOINT64(mysqlrow[0],RecordID);
	STRTOINT64(mysqlrow[1],requestID);
	STRTOINT64(mysqlrow[2],regionID);
	STRTOINT32(mysqlrow[3],regionoffset);
	STRTOINT64(mysqlrow[4],stripID);
	STRTOINT32(mysqlrow[5],stripoffset);
	STRTOINT64(mysqlrow[6],strsetID);
	STRTOINT32(mysqlrow[7],strsetoffset);
	STRTOINT64(mysqlrow[8],chunkID);
	STRTOINT32(mysqlrow[9],chunkoffset);
	STRTOINT32(mysqlrow[10],chunksize);
	//填一下
	checkpointptr->RecordID=RecordID;
	checkpointptr->requestID=requestID;
	checkpointptr->regionID=regionID;
	checkpointptr->regionoffset=regionoffset;
	checkpointptr->stripID=stripID;
	checkpointptr->stripoffset=stripoffset;
	checkpointptr->strsetID=strsetID;
	checkpointptr->strsetoffset=strsetoffset;
	checkpointptr->chunkID=chunkID;
	checkpointptr->chunkoffset=chunkoffset;
	checkpointptr->chunksize=chunksize;

	//chunklength=strlen(mysqlrow[6])+1;
	orgcontent=checkpointptr->orgcontent;
	mcpy(orgcontent,mysqlrow[11],char,chunksize);
	/*orgcontent=talloc(BYTE,chunklength);
	if(orgcontent==NULL)
	{
		show_error("mysql_FILELAYER","stripstripsectid_select","select cannot select");
		return FALSE;
	}*/
	//mcpy(orgcontent,mysqlrow[6],BYTE,chunklength);

	//chunklength=strlen(mysqlrow[7])+1;
	/*upcontent=talloc(BYTE,chunklength);
	if(upcontent==NULL)
	{
		show_error("mysql_FILELAYER","stripstripsectid_select","select cannot select");
		return FALSE;
	}*/
	upcontent=checkpointptr->upcontent;
	mcpy(upcontent,mysqlrow[12],char,chunksize);
	//mcpy(upcontent,mysqlrow[7],BYTE,chunklength);
	//填写日数据
...
//获取一个请求的所有块当前主要包括
int mysql_FILELAYER_checkpoint_select_group(CHECKPOINTGROUP *checkpointgrpptr,int reqID)
{
	UINT64 i,groupsize,RecordID,requestID,
		regionID,stripID,strsetID,chunkID;
	int regionoffset,stripoffset,strsetoffset,chunkoffset;
	...
	groupsize=checkpointgrpptr->groupsize;
	...
	//checkpointgrpptr->checkpointptr;
	sprintf(querystr,
		"select RecordID,requestID,regionID,regionoffset,stripID,stripoffset,strsetID,strsetoffset,chunkID,chunkoffset,chunksize,orgcontent,upcontent from checkpoint where RecordID=%I64d",
		requestID);
	mysqlresptr=mysql_FILELAYER_select_query(querystr);
	...
	for(i=0;i<groupsize;i++)
	{
		...
		STRTOINT32(mysqlrow[9],chunkoffset);
		STRTOINT32(mysqlrow[10],chunksize);
		curcheckpointptr->RecordID=RecordID;
		curcheckpointptr->requestID=requestID;

		curcheckpointptr->regionID=regionID;
		curcheckpointptr->stripID=stripID;
		curcheckpointptr->strsetID=strsetID;
		curcheckpointptr->chunkID=chunkID;

		curcheckpointptr->regionoffset=regionoffset;
		curcheckpointptr->stripoffset=stripoffset;
		curcheckpointptr->strsetoffset=strsetoffset;
		curcheckpointptr->chunkoffset=chunkoffset;

		curcheckpointptr->chunksize=chunksize;

		strlength=strlen(mysqlrow[11])+1;

		orgcontent=curcheckpointptr->orgcontent;
		mcpy(orgcontent,mysqlrow[11],char,strlength);

		strlength=strlen(mysqlrow[12])+1;

		upcontent=curcheckpointptr->upcontent;
		mcpy(upcontent,mysqlrow[12],char,1024);
...
//找到最大的请求和相应的个数
int mysql_FILELAYER_checkpoint_select_request_max_indexandcount(CHECKPOINTINF *checkpointinfptr)
{
	UINT64 requestID;
	int resTF,reqcount;
	...
	if(querystr==NULL)
	{
		show_error("mysql_FILELAYER","select_request_max_indexandcount","No memory");
		return FALSE;
	}
	sprintf(querystr,"select max(requestID) from checkpoint");
	
	mysqlresptr=mysql_FILELAYER_select_query(querystr);
	if(mysqlresptr==NULL)
	{
		show_error("mysql_FILELAYER","select_request_max_indexandcount","select can't execute");
		return FALSE;
	}
	//
	mysqlrow=mysql_fetch_row(mysqlresptr);
	if(mysqlrow==NULL)
	{
		show_error("mysql_FILELAYER","select_request_max_indexandcount","mysqlrow can't create");
		return FALSE;
	}
	STRTOINT64(mysqlrow[0],requestID);
	checkpointinfptr->reqindex=requestID;
	mysql_free_result(mysqlresptr);//第一个查询完了可以释放一下空间了
	//第二步找出这些数量可以直接调用方法
	resTF=mysql_FILELAYER_checkpoint_select_request_countnum(checkpointinfptr);
	if(resTF==FALSE)
	{
		show_error("mysql_FILELAYER","checkpoint_insert","count cannot calculate");
		return FALSE;
	}
	return TRUE;
}
//给出相应的请求index返回相应的个数
int mysql_FILELAYER_checkpoint_select_request_countnum(CHECKPOINTINF *checkpointinfptr)
{
	UINT64 requestID;
	int resTF,reqcount;
	...
	if(querystr==NULL)
	{
		show_error("mysql_FILELAYER","checkpoint_insert","No memory");
		return FALSE;
	}

	requestID=checkpointinfptr->reqindex;
	sprintf(querystr,"select count(requestID) from checkpoint where requestID=%I64d",requestID);
	mysqlresptr=mysql_FILELAYER_select_query(querystr);
	if(mysqlresptr==NULL)
	{
		show_error("mysql_FILELAYER_checkpoint","select_request_countnum","select cannot execute");
		return FALSE;
	}
	mysqlrow=mysql_fetch_row(mysqlresptr);
	if(mysqlrow==NULL)
	{
		show_error("mysql_FILELAYER_checkpoint","select_request_countnum","mysqlrow can't create");
		return FALSE;
	}
	STRTOINT32(mysqlrow[0],reqcount);
	checkpointinfptr->reqcount=reqcount;
	mysql_free_result(mysqlresptr);
	free(querystr);
	resTF=mysql_FILELAYER_checkpoint_select_request_chunksize(checkpointinfptr);
	if(resTF==FALSE)
	{
		show_error("mysql_FILELAYER_checkpoint","select_request_countnum","chunksize cannot obtain");
		return FALSE;
	}
	return TRUE;
}
int mysql_FILELAYER_checkpoint_select_request_chunksize(CHECKPOINTINF *checkpointinfptr)
{
	UINT64 requestID;
	int resTF,chunksize;
	...
	if(querystr==NULL)
	{
		show_error("mysql_FILELAYER","checkpoint_select_request_chunksize","No memory");
		return FALSE;
	}

	requestID=checkpointinfptr->reqindex;

	sprintf(querystr,"select chunksize from checkpoint where requestID=%I64d",requestID);
	mysqlresptr=mysql_FILELAYER_select_query(querystr);
	if(mysqlresptr==NULL)
	{
		show_error("mysql_FILELAYER","checkpoint_select_request_chunksize","select cannot execute");
		return FALSE;
	}
	mysqlrow=mysql_fetch_row(mysqlresptr);
	if(mysqlrow==NULL)
	{
		show_error("mysql_FILELAYER","checkpoint_select_request_chunksize","data cannot obtain");
		return FALSE;
	}
	STRTOINT32(mysqlrow[0],chunksize);
	checkpointinfptr->chunksize=chunksize;
	mysql_free_result(mysqlresptr);
	free(querystr);
	return TRUE;
}
...
int mysql_FILELAYER_checkpoint_update(CHECKPOINT *checkpointptr)
{
	int resTF;
	UINT64 RecordID;
	RecordID=checkpointptr->RecordID;
	resTF=mysql_FILELAYER_checkpoint_delete(RecordID);
	if(resTF==FALSE)
	{
		show_error("mysql_FILELAYER","userprofile_update","delete can't execute");
		return FALSE;
...
int mysql_FILELAYER_checkpoint_insert(CHECKPOINT *checkpointptr)
{
	int resTF,chunksize;
	UINT64 RecordID,requestID,
		regionID,stripID,strsetID,chunkID;
	...
	sprintf(querystr,
		"insert delayed into checkpoint(RecordID,requestID,regionID,regionoffset,stripID,stripoffset,strsetID,strsetoffset,chunkID,chunkoffset,chunksize,orgcontent,upcontent) values (%I64d,%I64d,%I64d,%d,%I64d,%d,%I64d,%d,%I64d,%d,%d,'%s','%s')",
		RecordID,requestID,regionID,regionoffset,stripID,stripoffset,strsetID,strsetoffset,chunkID,chunkoffset,chunksize,orgcontent,upcontent);
	
	resTF=mysql_FILELAYER_insert_query(querystr);
	if(resTF==FALSE)
	{
		show_error("mysql_FILELAYER","checkpoint_insert","insert cannot execute");
		return FALSE;
	}
...
int mysql_FILELAYER_checkpoint_delete(UINT64 RecordID)
{
	int resTF;
	char *querystr;
	querystr=talloc(char,1024);
	if(querystr==NULL)
	{
		show_error("mysql_FILELAYER","checkpoint_delete","No memory");
		return FALSE;
	}
	sprintf(querystr,
		"delete from checkpoint where recordID=%I64d",
		RecordID);
	resTF=mysql_FILELAYER_delete_query(querystr);
	if(resTF==FALSE)
	{
		show_error("mysql_FILELAYER","checkpoint_delete","delete can't execute");
		return FALSE;
	}
	free(querystr);
	return TRUE;
...
int mysql_FILELAYER_checkpoint_delete_request(UINT64 requestID)
{
	int resTF;
	char*querystr;
	...
	if(querystr==NULL)
	{
		show_error("mysql_FILELAYER","checkpoint_delete_request","No memory");
		return FALSE;
	}
	sprintf(querystr,
		"delete from checkpoint where requestID=%I64d",
		requestID);

	resTF=mysql_FILELAYER_delete_query(querystr);
	if(resTF==FALSE)
	{
		show_error("mysql_FILELAYER","checkpoint_delete_request","");
		return FALSE;
	}
	return TRUE;
}

int mysql_FILELAYER_checkpoint_delete_all()
{
	int resTF;
	char *querystr;
	querystr=talloc(char,1024);
	if(querystr==NULL)
	{
		show_error("mysql_FILELAYER","checkpoint_delete_all","No memory");
		return FALSE;
	}
	sprintf(querystr,"delete from checkpoint");
	resTF=mysql_FILELAYER_delete_query(querystr);
	if(resTF==FALSE)
	{
		show_error("mysql_FILELAYER","checkpoint_delete_all","delete can't execute");
		return FALSE;
	}
	return TRUE;
}
...
int mysql_FILELAYER_mysql_delete_all()
{
	...
	if(resTF==FALSE)
	{
		show_error("mysql_FILELAYER","mysql_delete_all","chunk cannot delete");
		return TRUE;
	}
	resTF=mysql_FILELAYER_device_delete_all();
	if(resTF==FALSE)
	{
		show_error("mysql_FILELAYER","mysql_delete_all","device cannot delete");
		return TRUE;
	}
	resTF=mysql_FILELAYER_devicediskid_delete_all();
	if(resTF==FALSE)
	{
		show_error("mysql_FILELAYER","mysql_delete_all","devicediskid cannot delete");
		return FALSE;
	}
	resTF=mysql_FILELAYER_deviceprocessid_delete_all();
	if(resTF==FALSE)
	{
		show_error("mysql_FILELAYER","mysql_delete_all","deviceprocessid cannot delete");
		return FALSE;
	}
	resTF=mysql_FILELAYER_deviceregionid_delete_all();
	if(resTF==FALSE)
	{
		show_error("mysql_FILELAYER","mysql_delete_all","deviceregionid cannot delete");
		return FALSE;
	}
	resTF=mysql_FILELAYER_disk_delete_all();
	if(resTF==FALSE)
	{
		show_error("mysql_FILELAYER","mysql_delete_all","disk cannot delete");
		return FALSE;
	}
	resTF=mysql_FILELAYER_diskregionid_delete_all();
	if(resTF==FALSE)
	{
		show_error("mysql_FILELAYER","mysql_delete_all","diskregionid cannot delete");
		return FALSE;
	}
