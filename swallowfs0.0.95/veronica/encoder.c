...
int encoder_MAPLAYER_update_map_data_and_pairty(CHUNKPOSITIONBLOCKGROUP *chunkpositionblockgroupptr,CHUNKBLOCKGROUP *parityblockgroupptr)
{
	...
	resTF=file_FILELAYER_CHUNKPOSITIONBLOCKGROUP_WRITER_DATA(chunkpositionblockgroupptr);
	if(resTF==FALSE)
	{
		show_error("encoder_MAPLAYER","update_map_data_and_pairty","CHUNKPOSITIONBLOCKGROUP cannot write");
		return FALSE;
	}

	resTF=file_FILELAYER_CHUNKBLOCKGROUP_WRITER_DATA(parityblockgroupptr);
	if(resTF==FALSE)
	{
		show_error("encoder_MAPLAYER","update_map_data_and_pairty","parityblockgroup cannot wirte");
		return FALSE;
	}
...
int encoder_filestore_API_update(char *tarpath,BYTESTREAM *updatestream,int offset)
{
	...
	//我们将处理完成的update requests先缓冲一份，如果最后写入失败可以用于恢复数据
	resTF=Logmode;
	//最后根据开关信息决定是否要存数据库,将原始数据存储数据库一旦崩溃
	if(resTF==LogOpen)
	{
		resTF=checkpoint_record_recent_request(chunkpositionblockgroupptr,parityblockgroupptr);
		if(resTF==FALSE)
		{
			show_error("encoder","filestore_API_update","update requests cannot record");
			return FALSE;
		}
	}
