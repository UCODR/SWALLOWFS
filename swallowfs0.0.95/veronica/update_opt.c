...
int UPDATE_OPT_calculate_UPDATEI(SCHEDULE *schedorder,CHUNKPOSITIONBLOCKGROUP *chunkpositionblockgroupptr,CHUNK *tarchunkptr,UPDATEELEMENT *updateelementptr)
{
	...
	if(resTF==FALSE)
	{
		show_error("UPDATE_OPT","calculate_UPDATEI","data can read");
		return TRUE;
	}
	...
	if(resTF==FALSE)
	{
		show_error("UPDATE_OPT","calculate_UPDATEI","data cannot calculate");
		return FALSE;
	}
...
int UPDATE_OPT_calculate_UPDATEII(SCHEDULE *schedorder,CHUNKPOSITIONBLOCKGROUP *chunkpositionblockgroupptr,CHUNK *tarchunkptr,UPDATEELEMENT *updateelementptr)
{
	...
	if(resTF==FALSE)
	{
		show_error("UPDATE_OPT","calculate_UPDATEII","data cannot read");
		return FALSE;
	}
	resTF=profile_XOR_CHUNKBLOCK_CHUNK_DATA(souchunkptr,tempchunkblockgroupptr);
	if(resTF==FALSE)
	{
		show_error("UPDATE_OPT","calculate_UPDATEII","data cannot calculate");
		return FALSE;
	}
	//将缓冲指针取出
	tempchunkptr=profile_updateelement_temp_chunk_ptr(updateelementptr);
