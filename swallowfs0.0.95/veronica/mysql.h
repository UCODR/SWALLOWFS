...
//根据reviewer意见加上checkpoint
extern int mysql_FILELAYER_checkpoint_select(CHECKPOINT *checkpointptr);
extern int mysql_FILELAYER_checkpoint_select_group(CHECKPOINTGROUP *checkpointgrpptr,int reqID);//获取一个请求的所有块当前主要包括
//不属于标准方法，为了回滚需要
extern int mysql_FILELAYER_checkpoint_select_request_max_indexandcount(CHECKPOINTINF *checkpointinfptr);//找到最大的请求和相应的个数
extern int mysql_FILELAYER_checkpoint_select_request_countnum(CHECKPOINTINF *checkpointinfptr);//给出相应的请求index返回相应的个数
extern int mysql_FILELAYER_checkpoint_select_request_chunksize(CHECKPOINTINF *checkpointinfptr);//根据requestID获取第一个记录的chunksize，并且写入checkinf
extern int mysql_FILELAYER_checkpoint_update(CHECKPOINT *checkpointptr);
extern int mysql_FILELAYER_checkpoint_insert(CHECKPOINT *checkpointptr);
extern int mysql_FILELAYER_checkpoint_delete(UINT64 RecordID);
extern int mysql_FILELAYER_checkpoint_delete_request(UINT64 requestID);
extern int mysql_FILELAYER_checkpoint_delete_all();
