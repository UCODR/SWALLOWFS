...
//#define UPDATESELECT UPDATE1INTERFACE
#define UPDATESELECT UPDATE2INTERFACE

//#define UPDATESELECT UPDATE1INTERFACE

#define MATRIXINTERFACE 0
#define UPDATEOPTINTERFACE 1
#define UPDATE1INTERFACE 2//RCW
#define UPDATE2INTERFACE 3//RMW
...
#define IOmode Paralmode

#define Seqmode 0
#define Paralmode 1


//#define Logmode LogClose
#define Logmode LogOpen

#define LogClose 0
#define LogOpen 1

//当前需要记录的请求个数
#define Requestlognumber 5

//定义存放路径
#define SYSBACKUPPATH "D:\\backup"
...
typedef struct CHECKPOINT
{
	...
	INT64 requestID;
	INT64 regionID;
	int regionoffset;//到第一个region的偏移量
	INT64 stripID;
	int stripoffset;
	INT64 strsetID;
	int strsetoffset;
...
typedef struct CHECKPOINTGROUP
{
	CHECKPOINT *checkpointptr;
	int groupsize;
}CHECKPOINTGROUP;

typedef struct CHECKPOINTINF
{
	UINT64 reqindex;//请求标号
	int reqcount;//有多少个块
	int chunksize;//需要知道块大小以便申请空间
}CHECKPOINTINF;
...
static UINT64 systemlogreqindex=0;//请求从0开始
