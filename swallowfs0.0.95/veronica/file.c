...
//原始读写方法
int file_FILELAYER_BYTE_READER_ORG(char *filepath,BYTE *content,int sizenum)
{
	FILE *pfile;
	int readsize,resTF;
	...
	if(pfile==NULL)
	{
		show_error("file_FILELAYER","BYTE_READER_ORG","file isn't exist");
		return FALSE;
	}
	readsize=fread(content,sizeof(BYTE),sizenum,pfile);
	if(readsize!=sizenum)
	{
		show_error("file_FILELAYER","BYTE_READER_ORG","content cannot read");
		return FALSE;
	}
...
int file_FILELAYER_BYTE_WRITER_ORG(char *filepath,BYTE *content,int sizenum)
{
	FILE *pfile;
	int readsize,resTF;
	...
	if(pfile==NULL)
	{
		show_error("file_FILELAYER","BYTE_WRITER_ORG","file isn't exist");
		return FALSE;
	}
	readsize=fwrite(content,sizeof(BYTE),sizenum,pfile);
	if(readsize!=sizenum)
	{
		show_error("file_FILELAYER","BYTE_WRITER_ORG","content cannot read");
		return FALSE;
	}
	fclose(pfile);
	return TRUE;
...
int file_FILELAYER_BYTE_DELETE_ORG(char *filepath)
{
	int resTF;
	resTF=remove(filepath);
	if(resTF!=QTRUE)//如果没有执行成功
	{
		show_error("file_FILELAYER","BYTE_DELETE_ORG","delete cannot execute");
		return FALSE;
	}
	return TRUE;
}
...
int file_FILELAYER_CHUNKGROUP_READER_DATA(CHUNKGROUP *chkgroup)
{
	...
	{
	case Seqmode:
		resTF=file_FILELAYER_CHUNKGROUP_READER_DATA_SEQ(chkgroup);
		if(resTF==FALSE)
		{
			show_error("file_FILELAYER","CHUNKGROUP_READER_DATA","data cannot read");
			return FALSE;
		}
		break;
	case Paralmode:
		resTF=file_FILELAYER_CHUNKGROUP_READER_DATA_PARAL(chkgroup);
		if(resTF==FALSE)
		{
			show_error("file_FILELAYER","CHUNKGROUP_READER_DATA","data cannot read");
			return FALSE;
...
int file_FILELAYER_CHUNKGROUP_READER_DATA_SEQ(CHUNKGROUP *chkgroup)
{
	int i,groupsize,resTF;
	CHUNK *chunkptr;
	chunkptr=chkgroup->ckgroup;
...
int file_FILELAYER_CHUNKGROUP_READER_DATA_PARAL(CHUNKGROUP *chkgroup)
{
	HANDLE *handleptr;
	CHUNK *chunkptr,*curchunkptr;
	int i,threadnum;
	//线程数等于块数
	threadnum=chkgroup->groupsize;
	handleptr=talloc(HANDLE,threadnum);
	if(handleptr==NULL)
	{
		show_error("file_FILELAYER","CHUNKGROUP_READER","No memory");
		return FALSE;
	}
	chunkptr=chkgroup->ckgroup;
	...
	{
		handleptr[i]=(HANDLE) _beginthreadex(NULL,0,readthread,curchunkptr,0,NULL);
		//i++;
		curchunkptr++;
	}
	//等待所有线程结束
	WaitForMultipleObjects(threadnum,handleptr,TRUE,INFINITE);
	free(handleptr);
	return TRUE;
}

int file_FILELAYER_CHUNKBLOCKGROUP_READER_DATA(CHUNKBLOCKGROUP *chkblkgroup)
{
	int resTF,typeindex;
	typeindex=IOmode;
	switch(typeindex)
	{
	case Seqmode:
		resTF=file_FILELAYER_CHUNKBLOCKGROUP_READER_DATA_SEQ(chkblkgroup);
		if(resTF==FALSE)
		{
			show_error("file_FILELAYER","CHUNKBLOCKGROUP_READER_DATA","data cannot read");
			return FALSE;
		}
		break;
	case Paralmode:
		resTF=file_FILELAYER_CHUNKBLOCKGROUP_READER_DATA_PARAL(chkblkgroup);
		if(resTF==FALSE)
		{
			show_error("file_FILELAYER","CHUNKBLOCKGROUP_READER_DATA","data cannot read");
			return FALSE;
...
int file_FILELAYER_CHUNKBLOCKGROUP_READER_DATA_SEQ(CHUNKBLOCKGROUP *chkblkgroup)
{
	int i,resTF;
	CHUNK *chunkptr;
	...
	chunkblockptr=chkblkgroup->chunkblockptr;
	groupsize=chkblkgroup->groupsize;
	...
	for(i=0;i<groupsize;i++)
	{
		...
		if(resTF==FALSE)
		{
			show_error("file_FILELAYER","CHUNKBLOCKGROUP_READER_DATA_SEQ","data cannot read");
			return FALSE;
		}
		curchunkblockptr++;
	}
...
int file_FILELAYER_CHUNKBLOCKGROUP_READER_DATA_PARAL(CHUNKBLOCKGROUP *chkblkgroup)
{
	//照方抓药再来一遍
	HANDLE *handleptr;
	CHUNK *chunkptr;
	CHUNKBLOCK *chunkblkptr,*curchunkblkptr;
	int i,threadnum;
	//线程数等于块数
	threadnum=chkblkgroup->groupsize;
	handleptr=talloc(HANDLE,threadnum);
	if(handleptr==NULL)
	{
		show_error("file_FILELAYER","CHUNKGROUP_READER","No memory");
	...
	chunkblkptr=chkblkgroup->chunkblockptr;
	curchunkblkptr=chunkblkptr;
	//chunkptr=chunkblkptr->chunkptr;
	for(i=0;i<threadnum;i++)
	{
		chunkptr=curchunkblkptr->chunkptr;
		handleptr[i]=(HANDLE) _beginthreadex(NULL,0,readthread,chunkptr,0,NULL);
		curchunkblkptr++;
	}
	WaitForMultipleObjects(threadnum,handleptr,TRUE,INFINITE);
	free(handleptr);
...
int file_FILELAYER_CHUNKPOSITIONBLOCKGROUP_DATA_READER(CHUNKPOSITIONBLOCKGROUP *chkposblkgroup)
{
	int resTF,typeindex;
	typeindex=IOmode;
	switch(typeindex)
	{
	case Seqmode:
		resTF=file_FILELAYER_CHUNKPOSITIONBLOCKGROUP_READER_DATA_SEQ(chkposblkgroup);
		if(resTF==FALSE)
		{
			show_error("file_FILELAYER","CHUNKPOSITIONBLOCKGROUP_READER_DATA","data cannot read");
			return FALSE;
		}
		break;
	case Paralmode:
		resTF=file_FILELAYER_CHUNKPOSITIONBLOCKGROUP_READER_DATA_PARAL(chkposblkgroup);
		if(resTF==FALSE)
		{
			show_error("file_FILELAYER","CHUNKPOSITIONBLOCKGROUP_READER_DATA","data cannot read");
			return FALSE;
...
int file_FILELAYER_CHUNKPOSITIONBLOCKGROUP_READER_DATA_SEQ(CHUNKPOSITIONBLOCKGROUP *chkposblkgroup)
{
	int i,resTF;
	int groupsize;
	CHUNK *chunkptr;
	CHUNKPOSITIONBLOCK *chunkposblkptr,*curchunkposblkptr;
	chunkposblkptr=chkposblkgroup->chunkblockptr;
	groupsize=chkposblkgroup->groupsize;

	curchunkposblkptr=chunkposblkptr;

	for(i=0;i<groupsize;i++)
	{
		chunkptr=curchunkposblkptr->chunkptr;
		resTF=file_FILELAYER_CHUNK_READER_DATA(chunkptr);
		if(resTF==FALSE)
		{
			show_error("file_FILELAYER","CHUNKPOSITIONBLOCKGROUP_READER_DATA_SEQ","data cannot read");
			return FALSE;
		}
		curchunkposblkptr++;
	}
...
int file_FILELAYER_CHUNKPOSITIONBLOCKGROUP_READER_DATA_PARAL(CHUNKPOSITIONBLOCKGROUP *chkposblkgroup)
{
	HANDLE *handleptr;
	CHUNK *chunkptr;
	CHUNKPOSITIONBLOCK *chunkpsoblkptr,*curchunkposblkptr;
	int i,threadnum;
	//线程数等于块数
	threadnum=chkposblkgroup->groupsize;
	handleptr=talloc(HANDLE,threadnum);
	if(handleptr==NULL)
	{
		show_error("file_FILELAYER","CHUNKGROUP_READER","No memory");
	...
	chunkpsoblkptr=chkposblkgroup->chunkblockptr;
	curchunkposblkptr=chunkpsoblkptr;
...
unsigned int __stdcall readthread(void *chkptr)//
{
	int resTF;
	CHUNK *chunkptr;
	chunkptr=(CHUNK *)chkptr;
	resTF=file_FILELAYER_CHUNK_READER_DATA(chunkptr);
	if(resTF==FALSE)
	{
		show_error("file_FILELAYER","CHUNKGROUP_READER","data cannot read");
		return FALSE;
	}
	return 0;
}
...
int file_FILELAYER_CHUNKGROUP_WRITER_DATA(CHUNKGROUP *chkgroup)
{
	...
	{
	case Seqmode:
		resTF=file_FILELAYER_CHUNKGROUP_WRITER_DATA_SEQ(chkgroup);
		if(resTF==FALSE)
		{
			show_error("file_FILELAYER","CHUNKGROUP_WRITER_DATA","Data cannot read");
			return FALSE;
		}
		break;
	case Paralmode:
		resTF=file_FILELAYER_CHUNKGROUP_WRITER_DATA_PARAL(chkgroup);
		if(resTF==FALSE)
		{
			show_error("file_FILELAYER","CHUNKGROUP_WRITER_DATA","Data cannot read");
			return FALSE;
...
//串行将CHUNKGROUP根据其中包含的路径写入到相应的区域,数据部分
int file_FILELAYER_CHUNKGROUP_WRITER_DATA_SEQ(CHUNKGROUP *chkgroup)
{
	int i,groupsize,resTF;
	CHUNK *chunkptr;
	chunkptr=chkgroup->ckgroup;
...
//
int file_FILELAYER_CHUNKGROUP_WRITER_DATA_PARAL(CHUNKGROUP *chkgroup)
{
	HANDLE *handleptr;
	int i,threadnum;
	CHUNK *chunkptr,*curchunkptr;
	...
	if(handleptr==NULL)
	{
		show_error("file_FILELAYER","CHUNKGROUP_WRITER","No memory");
		return FALSE; 
	}
	...
	for(i=0;i<threadnum;i++)
	{
		handleptr[i]=(HANDLE)_beginthreadex(NULL,0,writethread,curchunkptr,0,NULL);
		curchunkptr++;
	}
	WaitForMultipleObjects(threadnum,handleptr,TRUE,INFINITE);
	free(handleptr);
...
int file_FILELAYER_CHUNKBLOCKGROUP_WRITER_DATA(CHUNKBLOCKGROUP *chkblkgroup)
{
	int resTF,typeindex;
	typeindex=IOmode;
	switch(typeindex)
	{
	case Seqmode:
		resTF=file_FILELAYER_CHUNKBLOCKGROUP_WRITER_DATA_SEQ(chkblkgroup);
		if(resTF==FALSE)
		{
			show_error("file_FILELAYER","CHUNKBLOCKGROUP_WRITER_DATA","Data cannot read");
			return FALSE;
		}
		break;
	case Paralmode:
		resTF=file_FILELAYTER_CHUNBLOCKGROUP_WRITER_DATA_PARAL(chkblkgroup);
		if(resTF==FALSE)
		{
			show_error("file_FILELAYER","CHUNKBLOCKGROUP_WRITER_DATA","Data cannot read");
			return FALSE;
...
int file_FILELAYER_CHUNKBLOCKGROUP_WRITER_DATA_SEQ(CHUNKBLOCKGROUP *chkblkgroup)
{
	...
	chunkblockptr=chkblkgroup->chunkblockptr;
	chunknum=chkblkgroup->groupsize;
...
int file_FILELAYTER_CHUNBLOCKGROUP_WRITER_DATA_PARAL(CHUNKBLOCKGROUP *chkblkgroup)
{
	HANDLE *handleptr;
	int i,threadnum;
	CHUNK *chunkptr;
	CHUNKBLOCK *chunkblockptr,*curchunkblkptr;
	threadnum=chkblkgroup->groupsize;
	handleptr=talloc(HANDLE,threadnum);
	if(handleptr==NULL)
	{
	...
	chunkblockptr=chkblkgroup->chunkblockptr;
	curchunkblkptr=chunkblockptr;
	//curchunk=tr=chunkptr;
	for(i=0;i<threadnum;i++)
	{
		chunkptr=curchunkblkptr->chunkptr;
		handleptr[i]=(HANDLE)_beginthreadex(NULL,0,writethread,chunkptr,0,NULL);
		curchunkblkptr++;
	}
	WaitForMultipleObjects(threadnum,handleptr,TRUE,INFINITE);
	free(handleptr);
...
int file_FILELAYER_CHUNKPOSITIONBLOCKGROUP_WRITER_DATA(CHUNKPOSITIONBLOCKGROUP *chkposblkgroup)
{
	int resTF,typeindex;
	typeindex=IOmode;
	switch(typeindex)
	{
	case Seqmode:
		resTF=file_FILELAYER_CHUNKPOSITIONBLOCKGROUP_WRITER_DATA_SEQ(chkposblkgroup);
		if(resTF==FALSE)
		{
			show_error("file_FILELAYER","CHUNKPOSITIONBLOCKGROUP_WRITER_DATA","data cannot read");
			return FALSE;
		}
		break;
	case Paralmode:
		resTF=file_FILELAYER_CHUNKPOSITIONBLOCKGROUP_WRITER_DATA_PARAL(chkposblkgroup);
		if(resTF==FALSE)
		{
			show_error("file_FILELAYER","CHUNKPOSITIONBLOCKGROUP_WRITER_DATA","data cannot read");
			return FALSE;
...
int file_FILELAYER_CHUNKPOSITIONBLOCKGROUP_WRITER_DATA_SEQ(CHUNKPOSITIONBLOCKGROUP *chkposblkgroup)
{
	int i,resTF,chunknum;
	CHUNK *chunkptr;
	CHUNKPOSITIONBLOCK *chunkpositionblockptr,*curchunkpositionblockptr;

	chunkpositionblockptr=chkposblkgroup->chunkblockptr;
	chunknum=chkposblkgroup->groupsize;
...
int file_FILELAYER_CHUNKPOSITIONBLOCKGROUP_WRITER_DATA_PARAL(CHUNKPOSITIONBLOCKGROUP *chkposblkgroup)
{
	HANDLE *handleptr;
	int i,threadnum;
	CHUNK *chunkptr;
	CHUNKPOSITIONBLOCK *chunkposblockptr,*curchunposkblkptr;
	threadnum=chkposblkgroup->groupsize;
	handleptr=talloc(HANDLE,threadnum);
	if(handleptr==NULL)
	{
	...
	chunkposblockptr=chkposblkgroup->chunkblockptr;
	curchunposkblkptr=chunkposblockptr;
	//curchunk=tr=chunkptr;
	for(i=0;i<threadnum;i++)
	{
		chunkptr=curchunposkblkptr->chunkptr;
		handleptr[i]=(HANDLE)_beginthreadex(NULL,0,writethread,chunkptr,0,NULL);
		curchunposkblkptr++;
	}
	WaitForMultipleObjects(threadnum,handleptr,TRUE,INFINITE);
...
//回调函数
unsigned int __stdcall writethread(void *chkptr)
{
	CHUNK *chunkptr;
	int resTF;
	chunkptr=(CHUNK *)chkptr;
